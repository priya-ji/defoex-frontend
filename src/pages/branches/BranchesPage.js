    ...
    <div className="page-enter">
      <div className="page-header">
        ...
        {user?.role === 'superadmin' && (
          <button className="btn btn-primary" onClick={() => setShowCreate(true)}>+ Create Branch</button>
        )}
      </div>
          ...
          <div className="branches-grid">
            {branches.map(b => (
              <div key={b.id} className="branch-card">
                ...
                <div className="branch-card__body">
                  ...
                  {b.manager_email && <div className="branch-row"><span>✉️</span><span>{b.manager_email}</span></div>}
                </div>
                <div className="branch-card__footer">
                  <button className="btn btn-outline btn-sm" onClick={() => openDetail(b)}>View Details</button>
                  {user?.role === 'superadmin' && (
                    <button className="btn btn-primary btn-sm" onClick={() => { setTopupBranch(b); setShowTopup(true); }}>
                      + Top Up
                    </button>
                  )}
                </div>
            ...
            {!branches.length && (
              <div className="empty-state">
                ...
                {user?.role === 'superadmin' && (
                  <button className="btn btn-primary mt-2" onClick={() => setShowCreate(true)}>+ Create Branch</button>
                )}
              </div>
            )}
          </div>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create New Branch" size="lg">
        ...
        <div style={{borderTop:'1px solid var(--border)',paddingTop:16,marginTop:4}}>
          <div style={{fontSize:'0.78rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.5px',color:'var(--primary)',marginBottom:12}}>Branch Manager</div>
          <div className="reg-form-row">
        ...
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:16}}>
          ...
          <button className="btn btn-primary" onClick={createBranch} disabled={saving}>{saving ? 'Creating...' : 'Create Branch'}</button>
        </div>
      </Modal>
      ...
      <Modal open={showTopup} onClose={() => setShowTopup(false)} title={`Top Up — ${topupBranch?.branch_name}`} size="sm">
        <Field label="Amount (₹)" required>
          <Input type="number" value={topupAmt} onChange={e => setTopupAmt(e.target.value)} placeholder="e.g. 500000" autoFocus />
        </Field>
        ...
        <Alert type="warning" className="mt-2">This adds to the branch current_balance. The BM can then create investment plans up to this amount.</Alert>
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:16}}>
          <button className="btn btn-outline" onClick={() => setShowTopup(false)}>Cancel</button>
          <button className="btn btn-primary" onClick={doTopup} disabled={saving}>{saving ? 'Processing...' : `Add ${topupAmt ? fmt(topupAmt) : 'Funds'}`}</button>
        </div>
      </Modal>
